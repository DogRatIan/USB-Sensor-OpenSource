...
// Constants
//==========================================================================
static const struct TBaudTable KBaudTable[] = {{"1200", CBR_1200, 1200},
                                               {"2400", CBR_2400, 2400},
                                               {"4800", CBR_4800, 4800},
                                               {"9600", CBR_9600, 9600},
...
static const struct TModeTable KModeTable[] = {{"N81", 8, NOPARITY, ONESTOPBIT},
...
// Create of Object
//==========================================================================
CSerial::CSerial(void) : BaudRate(9600), Mode("N81"), ErrorMessage(NULL), ComFid(INVALID_HANDLE_VALUE) { strcpy(PortName, "COM1"); }
...
const char *CSerial::SearchCfg(const char *aCfg, const char *aStr) {
  ...
  while ((strncmp(aCfg, aStr, len)) && (aCfg[0] != 0)) aCfg++;

  if (aCfg[0] == 0) return NULL;
...
int CSerial::Open(const char *aPortName, const char *aCfg) {
  ...
  if (ComFid == INVALID_HANDLE_VALUE) {
    ComFid = CreateFile(PortName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
  } else {
    ErrorMessage = "Port already opened.";
    return -1;
  }

  if (ComFid == INVALID_HANDLE_VALUE) {
    ErrorMessage = "Fail to open port.";
    return -1;
  }

  if (!GetCommState(ComFid, &dcb))  // Config Port
  {
    ErrorMessage = "Get DCB Error.";
    return -1;
  }
  ...
  if (!SetCommTimeouts(ComFid, &timeout)) {
    ErrorMessage = "Set Timeout failed.";
    return -1;
  }

  // Get properties
  if (!GetCommProperties(ComFid, &prop)) {
    ErrorMessage = "Get properties failed.";
    return -1;
  }
...
void CSerial::Flush(void) {
  if (ComFid == INVALID_HANDLE_VALUE) return;
...
bool CSerial::Dsr(void) {
  ...
  if (ComFid == INVALID_HANDLE_VALUE) return false;

  if (GetCommModemStatus(ComFid, &status) == FALSE) return false;
